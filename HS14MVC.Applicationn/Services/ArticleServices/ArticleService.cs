...
namespace HS14MVC.Applicationn.Services.ArticleServices
{
    public class ArticleService : IArticleService
    {
        ...
        {
            var updatingArticle = await _articleRepository.GetByIdAsync(articleUpdateDTO.Id, false);
            if (updatingArticle == null)
            {
                return new ErrorDataResult<ArticleDTO>("Güncellenecek Article Bulunamadı");
            }

            var updatedArticle = articleUpdateDTO.Adapt(updatingArticle);
            await _articleRepository.UpdateAsync(updatedArticle);
            await _articleRepository.SaveChangesAsync();


            return new SuccessDataResult<ArticleDTO>(updatedArticle.Adapt<ArticleDTO>(), "Article Güncelleme Başarılı");
        }
